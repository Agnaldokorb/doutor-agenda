...
import { boolean, foreignKey, integer, pgEnum,pgTable, text, time, timestamp, unique, uuid } from "drizzle-orm/pg-core"

export const appointmentStatus = pgEnum("appointment_status", ['agendado', 'confirmado', 'cancelado', 'concluido'])
...
export const appointments = pgTable("appointments", {
	...
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
	status: appointmentStatus().default('agendado').notNull(),
}, (table) => [
	foreignKey({
			columns: [table.clinicId],
			foreignColumns: [clinics.id],
